...
function unArchiveBulk() {
	if(!empty($_POST['check'])){
		foreach ($_POST['check'] as $checkbox) {
			...
			$query = "UPDATE wp_iecrm_customers SET status=0 WHERE id='".$checkbox."'";
			$result = mysqli_query($con,$query);
		}
		
		$message = "Customers Unarchived";
	    echo "<script type='text/javascript'>alert('$message');</script>";
	}
...
<div class="wrap" id="customerList">
	<form action="#" method="post">
		<span class="contact100-form-title">Customer Archive List</span>
		<div class="container-table100">
			<div class="wrap-table100">
				...
				<div style="float: right;">
					<input type="button" class = "contact100-form-btn" value="Search" onclick="window.location.href='admin.php?page=customer-archive' + inputCheck();">
				</div>
				<div style="float: right;">
					...
					<table data-vertable="ver1">
						<thead>
							<tr class="row100 head">
								...
								<th class="column100 column3" data-column="column3"><a href = "admin.php?page=customer-archive&Val=ID<?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>">ID</a></th>
								<th class="column100 column4" data-column="column4"><a href = "admin.php?page=customer-archive&Val=name<?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>">NAME</a></th>
								<th class="column100 column5" data-column="column5"><a href = "admin.php?page=customer-archive&Val=phone">PHONE</a></th>
								<th class="column100 column6" data-column="column6"><a href = "admin.php?page=customer-archive&Val=companyName">COMPANY NAME</a></th>
				...
				//PHP shiz to set sort-by values and current page of results (default atm = date newest to oldest)
						$orderBy = array('ID', 'name', 'phone', 'companyName', 'abn');
						$limitMin = 0;
						$limitMax = 10;
						...
						foreach ( $result as $print ) {  
							...
							<tbody>
								<tr class="row100">
									...
									<td class="column100 column6" data-column="column6"> <?php echo ($print->companyName == "" ? "N/A" : $print->companyName); ?></td>
									<td class="column100 column7" data-column="column7"><input type="button" class = "contact100-form-btn" value="View/Edit" onclick="window.location.href='admin.php?page=customer-archive&Edit=<?php echo $print->ID; ?>'"></td>
								</tr>
					...
					<div style="float: right;">
						<input type="button" class = "contact100-form-btn" value="Back" onclick="window.location.href='admin.php?page=customer-archive<?php echo (isset($_GET['Val']) ? "&Val=".$_GET['Val']: ''); ?><?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>&Asc=<?php echo (isset($_GET['Asc']) && $_GET['Asc'] > 0 ? $_GET['Asc'] - 10: 0); ?>'" style="width: 100px;">
						<input type="button" class = "contact100-form-btn" value="Next" onclick="window.location.href='admin.php?page=customer-archive<?php echo (isset($_GET['Val']) ? "&Val=".$_GET['Val']: ''); ?><?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>&Asc=<?php echo (isset($_GET['Asc']) && $_GET['Asc'] > 0 ? $_GET['Asc'] + 10: 10); ?>'" style="width: 100px;">
					</div>
			</div>
		</div>
	</form>
