...
function unArchiveBulk() {
	if(!empty($_POST['check'])){
		foreach ($_POST['check'] as $checkbox) {
			...
			$query = "UPDATE wp_iecrm_vehicles SET status=0 WHERE ID='".$checkbox."'";
			$result = mysqli_query($con,$query);
		}
		
		$message = "Vehicles Unarchived";
	    echo "<script type='text/javascript'>alert('$message');</script>";
	}
...
<div class="wrap" id="vehicleList">
	<form action="#" method="post">
		<span class="contact100-form-title">Vehicle Archive List</span>
		<div class="container-table100">
			<div class="wrap-table100">
				<input type="button" class = "contact100-form-btn" value="Add New Vehicle" onclick="window.location.href='admin.php?page=vehicle-archive'" style="width: 15%;  background:#0073aa;">
				<div style="float: right;">
					<input type="button" class = "contact100-form-btn" value="Search" onclick="window.location.href='admin.php?page=vehicle-archive' + inputCheck();">
				</div>
				<div style="float: right;">
					...
					<table data-vertable="ver1">
						<thead>
							<tr class="row100 head">
								...
								<th class="column100 column3" data-column="column3"> <a href = "admin.php?page=vehicle-archive&Val=ID<?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>"> ID</a></th>
								<th class="column100 column4" data-column="column4"> <a href = "admin.php?page=vehicle-archive&Val=name<?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>">NICKNAME</a></th>
								<th class="column100 column5" data-column="column5"> <a href = "admin.php?page=vehicle-archive&Val=license<?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>">LICENSE PLATE</a></th>
								<th class="column100 column6" data-column="column6"> <a href = "admin.php?page=vehicle-archive&Val=ton<?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>">VEHICLE WEIGHT</a></th>
								<th class="column100 column7" data-column="column6"> <a href = "admin.php?page=vehicle-archive&Val=status<?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>">STATUS</a></th>
				...
				//PHP shiz to set sort-by values and current page of results (default atm = date newest to oldest)
						$orderBy = array('ID', 'name', 'license', 'ton', 'status');
						$limitMin = 0;
						$limitMax = 100;
						...
						foreach ( $result as $print ) {  
							...
							<tbody>
								<tr class="row100">
									...
									<td class="column100 column6" data-column="column6"> <?php echo ($print->ton == '0' ? '4T' : ($print->ton == '1' ? '8T' : ($print->ton == '2' ? '12T' : ($print->ton == '3' ? 'Other' : 'Error')))); ?></td>
									<td class="column100 column6" data-column="column6">Archived</td>
									<td class="column100 column7" data-column="column7"><input type="button" class = "contact100-form-btn" value="View/Edit" onclick="window.location.href='admin.php?page=vehicle-archive&Edit=<?php echo $print->ID; ?>'"></td>
								</tr>
				...
				<div style="float: left;">
					...
					<select name="bulkActions" class="js-select2">
						...
						<option value="UnArchiveBulk">Unarchive</option>
					</select>
				...
				<div style="float: right;">
					<input type="button" class = "contact100-form-btn" value="Back" onclick="window.location.href='admin.php?page=vehicle-archive<?php echo (isset($_GET['Val']) ? "&Val=".$_GET['Val']: ''); ?><?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>&Asc=<?php echo (isset($_GET['Asc']) && $_GET['Asc'] > 0 ? $_GET['Asc'] - 10: 0); ?>'" style="width: 100px;">
					<input type="button" class = "contact100-form-btn" value="Next" onclick="window.location.href='admin.php?page=vehicle-archive<?php echo (isset($_GET['Val']) ? "&Val=".$_GET['Val']: ''); ?><?php echo (isset($_GET['Search']) ? "&Search=".$_GET['Search']: ''); ?>&Asc=<?php echo (isset($_GET['Asc']) && $_GET['Asc'] > 0 ? $_GET['Asc'] + 10: 10); ?>'" style="width: 100px;">
				</div>
			</div>
		</div>
	</form>
